...
def generate_readme_with_llm(language, slug, description, api_key):
    ...
    prompt = f"""Write a professional, detailed GitHub README.md for a {lang_label} project.

Project name: {slug}
Description: {description}

The README must include these sections in order:
1. A top title (# {slug}) with a one-line tagline
2. Badges line: language badge, license MIT badge, status active badge (use shields.io)
3. ## Overview — 3-4 sentences describing the problem it solves and why it's useful
4. ## Features — bullet list of 4-6 specific features
5. ## Requirements — prerequisites (Python 3.8+ or Rust 1.70+, any key libraries)
6. ## Installation — step-by-step with code blocks using: {install_hint}
7. ## Usage — clear example with a code block using: {run_hint}
8. ## Example Output — a realistic sample output the tool would produce
9. ## Contributing — one short paragraph
10. ## License — MIT

Output ONLY the raw markdown. No explanations outside the markdown."""

    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    data = {
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": "You are a technical writer who writes clean, professional GitHub READMEs."},
            {"role": "user", "content": prompt}
        ],
        "temperature": 0.4
    }
...
def main():
    ...
    # 1. README — generated by LLM for a proper, professional README
    readme_content = generate_readme_with_llm(lang, slug, desc, api_key)
